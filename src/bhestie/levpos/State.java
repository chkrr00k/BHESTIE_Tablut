...
public class State {
	 ...
	 * other positions = 2
	 */
	private int threatenKing(Pawn king){
		int startingValue;
		...
		threatenPositions.add(new Position(king.getX() + 1, king.getY()));
	...
	public boolean isTerminal() {
		if (this.drawCase)
			return true;
		if (!this.pawns.stream().anyMatch(p -> p.bw == true /*black*/)) // No more black pawns
			return true;
		Optional<Pawn> king = this.pawns.stream().filter(p -> p.king).findAny();
